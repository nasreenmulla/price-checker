...
GROUP BY COM_NO, STR_CODE, POS.ITM_CODE, REF_CODE,DESC_E
ORDER BY COM_NO, STR_CODE, ITM_CODE";
echo"<table><tr><td>com</td> <td>str</td> <td>itm</td> <td>ref</td> <td>DESC</td> <td>qty</td> </tr>";
  $stid = oci_parse($conn, $query);
  ...
  while (($row = oci_fetch_row($stid)) != false) {
    ...
    // while (($row1 = oci_fetch_row($stid1)) != false) { 
      ...
      echo"<tr>";
echo "<td>".$row[0]."</td><td>".$row[1]."</td><td>".$row[2]."</td><td>".$row[3]."</td><td>".$row[4]."</td><td>".$row[5]."</td><td>".$row[3]."</td>";
echo"</tr>";
